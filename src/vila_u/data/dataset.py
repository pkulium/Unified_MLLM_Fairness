...
class LazyGenerationDataset(Dataset):
    ...
    def __init__(
        ...
        try:  
            ...
            with open(data_path, "r") as fp:
                self.dataset = [json.loads(q) for q in fp]
            
        # v5
        if DEBUG:
            self.dataset = [item for item in self.dataset if 'img_0.jpg' in item['.jpg']][:100]
...
class LazyGenerationBPODataset(Dataset):
    ...
    def __init__(
        ...
        try:  
         ...
         # v5
        if DEBUG:
            self.dataset = [item for item in self.dataset if 'img_0.jpg' in item['.jpg'][0]][:100]
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        info = self.dataset[i]
        key, image_path, json_file = info["__key__"], info[".jpg"], info[".json"]
        image_path_pref1, image_path_pref2 = image_path[0], image_path[1]
        ...
        sources = [conversation]
        image_pref1 = process_image(image_path_pref1, self.data_args, image_folder=None, generation_mode=True)
        image_pref2 = process_image(image_path_pref2, self.data_args, image_folder=None, generation_mode=True)
        sources = preprocess_multimodal(copy.deepcopy(sources), self.data_args)
        ...
        data_dict["image"] = image_pref1.unsqueeze(0)
        data_dict["image_extra"] = image_pref2.unsqueeze(0)
...
# @dataclass
# class DataCollatorForSupervisedBPODataset(object):
...
#         for _images, _input_ids in zip(images, input_ids):
#             assert (
#                 len(_images[0]) == (_input_ids == IMAGE_TOKEN_INDEX).sum().item()
#             ), f"Number mismatch between images and placeholder image tokens in 'len(_images) == (_input_ids == IMAGE_TOKEN_INDEX).sum().item()'.\
#                 Expect to have {len(_images)} images but only found {(_input_ids == IMAGE_TOKEN_INDEX).sum().item()} images in tokens. \
#                 Error input_ids: {_input_ids}"
...
#             num_images = (input_ids[ix] == IMAGE_TOKEN_INDEX).sum().item()
#             cur_images = images[ix]
#             cur_images = cur_images[:num_images * 2]
#             if len(cur_images) > 0:
#                 new_images.append(cur_images)
...
class DataCollatorForSupervisedBPODataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        for instance in instances:
            ...
            if instance["image"] is not None:
                cur_image = instance["image"]
                cur_image_extra = instance["image_extra"]

                assert len(cur_image.shape) == 4

                if not isinstance(instance["input_ids"], list):
                    images.append(cur_image)
                    images.append(cur_image_extra)
                else:
                    images.extend(cur_image.chunk(cur_image.size(0), 0))
                    images.extend(cur_image_extra.chunk(cur_image.size(0), 0))
            else:
                images.append([])
        ...
        # This creates something like [0, 0, 1, 1, 2, 2, ..., B-1, B-1]
        indices = torch.arange(batch_size).repeat_interleave(2)

        # Apply the index to replicate each row
        input_ids = input_ids[indices]        # => shape [2B, T]
        labels = labels[indices]             # => shape [2B, T]
...
def make_supervised_data_module(
    ...
    if 'bpo' in data_args.data_mixture:
        data_collator = DataCollatorForSupervisedBPODataset(tokenizer=tokenizer, data_args=data_args)
    else:
...
def build_datasets(
    ...
    for dataset in mixture:
        ...
        elif dataset_type == "internal-generation-bpo":
            dataset_cls = LazyGenerationBPODataset
