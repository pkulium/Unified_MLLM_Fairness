...
class CustomCausalLMOutputWithPast(CausalLMOutputWithPast):
    image_logits: torch.LongTensor = None
    image_labels: torch.LongTensor = None
...
class VILAULlamaModel(VILAUMetaModel, VILAUMetaForCausalLM, PreTrainedModel):
    ...
    def forward(
        ...
        if self.beta:
            bpo_loss, _, _ = self.bpo_loss_func(image_logits.reshape(B, seq_len, D, C), image_labels.reshape(B, -1))

            print("=" * 32 + "bpo_loss" + "=" * 32)
            print("bpo_loss: ", bpo_loss)
            print("finetune_loss: ", loss)
