...
class VILAUMetaForCausalLM(ABC):
    ...
    @torch.inference_mode()
    def get_image_embeddings(self, prompt: Union[str, List], generation_config: Optional[GenerationConfig] = None) -> str:
        conversation = [{"from": "human", "value": prompt}]
        media = extract_media(conversation, self.config)
        ...
        image_features, tokens = self.encode_images(images, None)
        return image_features, tokens

    @torch.inference_mode()
    def generate_content_batch(self, prompt: Union[str, List], generation_config: Optional[GenerationConfig] = None) -> str:
        def generate_in_batches(self, input_ids, images, generation_config, batch_size=16):
            all_output_ids = []
            total_sequences = generation_config.num_return_sequences
            
            for i in range(0, total_sequences, batch_size):
                # Adjust batch size for the last iteration if necessary
                current_batch_size = min(batch_size, total_sequences - i)
                
                # Create a copy of generation_config for this batch
                batch_config = copy.deepcopy(generation_config)
                batch_config.num_return_sequences = current_batch_size
                
                # Generate for the current batch
                batch_output_ids = self.generate(input_ids=input_ids, images=images, generation_config=batch_config)
                
                all_output_ids.append(batch_output_ids)
            
            # Concatenate all batches into a single tensor
            return torch.cat(all_output_ids, dim=0)
        ...
        # output_ids = generate_in_batches(self, input_ids, images, generation_config)


        # if generation_config.num_return_sequences:
        #     # Decode the response
        #     output_ids_cpu = output_ids.cpu()
        #     response = self.tokenizer.batch_decode(output_ids_cpu, skip_special_tokens=True)
        # else:
            # response = self.tokenizer.decode(output_ids[0], skip_special_tokens=True).strip()
    ...
    def generate_image_content(self, prompt: str, cfg: float = 3.0, generation_nums: int = 1) -> torch.Tensor:
        ...
        return response.chunk(2)[0], image_ids, image_embeds
